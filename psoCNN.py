...
class psoCNN:
    def __init__(self, dataset, n_iter, pop_size, batch_size, epochs, min_layer, max_layer, \
        ...
        if dataset == "FaceShape_Dataset":
          input_width = 28
          input_height = 28
          input_channels = 3
          output_dim = 5

          path_train = "/content/drive/My Drive/CS410/psoCNN/FaceShape_Dataset/training_set"
          path_test = "/content/drive/My Drive/CS410/psoCNN/FaceShape_Dataset/testing_set"
          for i,folder_name in enumerate(os.listdir(path_train)):
            print(i)
            print(folder_name)
            folder_path = os.path.join(path_train, folder_name)
            folder_path_test = os.path.join(path_test, folder_name)
            for fname in os.listdir(folder_path):
              fpath = os.path.join(folder_path, fname)
              #print(fpath)
              #img = cv2.imread(fpath)
              #print(img)
              img = image.load_img(fpath,target_size=(28,28))
              #img = img[:,:,0]
              img_data = image.img_to_array(img)
              self.x_train.append(img_data)
              self.y_train.append(i)
            for fname in os.listdir(folder_path_test):
              fpath = os.path.join(folder_path_test, fname)
              #print(fpath)
              #img = cv2.imread(fpath)
              #print(img)
              img = image.load_img(fpath, target_size=(28,28))
              #img = img[:,:,0]
              img_data = image.img_to_array(img)
              self.x_test.append(img_data)
              self.y_test.append(i)
            print("Complete a class")
          self.x_train = np.array(self.x_train)
          self.y_train = np.array(self.y_train)
          self.x_test = np.array(self.x_test)
          self.y_test = np.array(self.y_test)
          print(self.x_train.shape[3])
          print(self.x_test.shape[3])
          #self.x_train /= 255.0
